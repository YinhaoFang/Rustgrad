...
broadcasting, transpose, reshape, sum, mean, axis reductions, matrix
multiplication, and row-wise bias addition (`row_add`). The shape checks are
part of the technical design, not only defensive code. Later modules rely on
these checks to reject invalid training inputs before they can produce
misleading results.
...
`backward_with_grad(output, seed)` is a key extension added in 0.2.0. Training
loops compute the loss gradient analytically (a short, auditable formula), then
pass it as a seed to the autograd engine. The engine propagates through
activations, linear transforms, and intermediate operations to produce parameter
gradients. `take_gradients()` extracts accumulated gradients from trainable
leaf nodes in insertion order, ready for the optimizer. This split — analytical
loss gradient plus automatic parameter gradients — keeps the training code both
verifiable and general.
...
The `serialize` module (added in 0.2.0) provides plain-text model checkpoint
persistence. Each tensor is written as a header line (`rank,dim0,…`) followed
by one `f64` value per line with 17 significant digits. `save_linear` /
`load_linear` and `save_xor_mlp` / `load_xor_mlp` provide model-level
round-trip. The CLI exposes this through `--save-model PATH` on all three
training commands, auto-creating parent directories.

The `data` module was extended with three utilities in 0.2.0.
`Dataset::from_csv` parses CSV text with header and comment-line support.
`Dataset::shuffle(seed)` performs a deterministic Fisher-Yates shuffle using a
linear congruential generator — same seed, same permutation, reproducible
across machines. `Dataset::split(ratio)` partitions into train and test
subsets. `Dataset::iter_rows()` provides an `ExactSizeIterator` over
`(features, targets)` row pairs.

The `backend` module (added in 0.2.0) defines a `Backend` trait covering all
core tensor operations. `CpuBackend` implements the trait as a zero-cost
delegation to `Tensor` methods. The trait is object-safe, so future GPU
backends can slot in without API changes.
...
`--save-model PATH` for checkpoint export:

```bash
cargo run -- train-xor --epochs 160 --save-model runs/xor.checkpoint
```

It can also export report files:
...
RustGrad's current limits, documented in the 0.2.0 changelog:

- GPU backend is architectural (trait + CpuBackend); no actual kernels.
- Serialization covers `Linear` and `XorMlp`, not generic `Sequential`
  containers. No `--load-model` CLI flag.
- `ComputationGraph` is rebuilt from scratch each epoch.
- Softmax in `train_spiral_classifier` does not participate in the backward
  graph (the simplified softmax+CE combined gradient seeds logits directly).
- All training examples use full-batch gradient descent.

These choices keep the project suitable for a course experiment and make the
...
置，reshape，求和，均值，按轴归约，矩阵乘法，以及行方向偏置加法
（`row_add`）。shape 检查是技术设计的一部分。后续模块依赖这些检查，在训
练输入无效时尽早返回错误，减少误导性结果。
...
`backward_with_grad(output, seed)` 是 0.2.0 新增的关键接口。训练循环解析计
算损失梯度（公式短小、易审查），将其作为种子传入 autograd 引擎，引擎再自动
传播经过激活函数、线性变换和中间操作，最终产生参数梯度。`take_gradients()`
按插入顺序取出所有可训练叶子节点的累积梯度，直接供优化器使用。这种"损失梯
度手工 + 参数梯度自动"的拆分，兼顾了教学可验证性和框架通用性。
...
`serialize` 模块（0.2.0 新增）提供了纯文本模型 checkpoint 持久化。每个张量
写为一行标头（`rank,dim0,…`）+ 逐行 f64 值（17 位有效数字）。`save_linear`
/ `load_linear` 和 `save_xor_mlp` / `load_xor_mlp` 提供模型级回环读写的
API。CLI 通过 `--save-model PATH` 将此能力暴露给三个训练命令，自动创建父目
录。

`data` 模块在 0.2.0 中扩展了三个工具。`Dataset::from_csv` 解析 CSV 文本，支
持表头行和 `#` 注释行。`Dataset::shuffle(seed)` 使用线性同余生成器执行确定
性 Fisher-Yates 洗牌——相同 seed, 相同排列, 跨机器可复现。`Dataset::split(ratio)`
按比例划分为训练/测试子集。`Dataset::iter_rows()` 提供 `ExactSizeIterator`,
惰性产出 `(features, targets)` 行向量对。

`backend` 模块（0.2.0 新增）定义了覆盖全部核心张量运算的 `Backend` trait。
`CpuBackend` 以零成本委托到 `Tensor` 方法实现该 trait。trait 是 object-safe
的，未来 GPU 后端可在不改变 API 的情况下接入。
...
RustGrad 的当前限制（0.2.0 已知限制）：

- GPU 后端仅有 trait 接口，未实现实际 GPU kernel。
- 序列化覆盖 Linear 和 XorMlp，未覆盖泛化 Sequential 容器。CLI 未提供
  `--load-model` 选项。
- `ComputationGraph` 每 epoch 从零重建，未跨步骤复用。
- `train_spiral_classifier` 中 Softmax 不参与反向传播图（简化的 softmax+CE
  组合梯度直接种子 logits 节点）。
- 所有训练示例使用全批量梯度下降。

这些选择让项目适合作为课程实验，也让行为更容易验证。
